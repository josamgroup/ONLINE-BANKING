...
public function borrow_loan(Request $request)
    {
        ...
        echo $id = $request->input('client_id');
        $client = Client::find($id);
        print_r($client);exit();
        if (empty($client)) {
           return response()->json(['msg' => 'No records found Please contact us for more details.', 'status' => '200']);
        ...
        // \flash(trans_choice("core::general.successfully_saved", 1))->success()->important();
        // return redirect('portal/loan/application');
        return response()->json(['msg' => 'Loan application successful, You will be notified on other steps.Thank you ,', 'status' => '200','data' =>'']);
    }
