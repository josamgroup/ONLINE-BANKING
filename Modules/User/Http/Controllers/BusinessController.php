   ...
   public function sendSms($phone,$message){
    ...
    public function index(Request $request)
    {
       ...
       // echo '<pre>';print_r(Auth::user());exit();
        ...
        $data = Business::when($orderBy, function (Builder $query) use ($orderBy, $orderByDir) {
            $query->orderBy($orderBy, $orderByDir);
        })
            ->when($search, function (Builder $query) use ($search) {
                $query->where('name', 'like', "%$search%");
                $query->orWhere('orgid', 'like', "%$search%");
            ...
            // print_r($data);
            // exit;
        return theme_view('user::business.index', compact('data'));
    }
    ...
    public function create()
    {
        ...
        $roles = Role::all();
        return theme_view('user::business.create', compact('custom_fields', 'roles'));
    }
...
//echo 
        ...
        $vcode = substr(str_shuffle($permitted_char), 0, 16);
        ...
        $msg ="Dear ".$request->first_name.' '.$request->last_name.". We have created Admin Account for ".$request->name." ,Please use: ".$pass."  as your password with your email.";

        $this->sendSms("254".(int)$request->phone,$msg);
        ...
        $business = Business::create($creds);
    ...
    public function edit($id)
    {
        ...
        // })->toArray();
        $custom_fields = CustomField::where('category', 'add_user')->where('active', 1)->get();
        return theme_view('user::business.edit', compact('user', 'selected_roles', 'custom_fields', 'roles'));
    }
    ...
    {
        $user = Auth::user();
        return theme_view('user::user.profile.reset_password', compact('user'));
    }
    ...
    public function update_password(Request $request)
    {
        ...
        if (!Hash::check($request->current_password, $user->password)) {
            ...
            Session::flash('error',trans_choice("core::general.Invalid Password", 1));//->error()->important();
            return redirect()->back();
        }
        $credentials = [
