        ...
        $user = DB::table('users')->where('email', $email)->first();
           ...
           if($user){
            ...
            if(password_verify($request->password, $pass_hash)){
                ...
                //$this->sendSms($user->phone,$msg); --For sending sms after successful login

                 //return theme_view('user::auth.passwords.otp',compact('user','email','password')); //redirect to OTP check
                 //Auth::guard('api')->login($users);

                 Auth::login($users); //pass the session and skip 2fa on UI
             ...
             Session::flash('error','Wrong Credentials. <br> Provided credentials does not match our records');
             return redirect()->back();
        }
...
//print_r($data);
    ...
    protected function switch(Request $request)
    {
