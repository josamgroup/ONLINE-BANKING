...
use Modules\Loan\Entities\LoanTrans;
...
public function getWallet($id)
          {
            $record =LoanWallets::where('CustomerID', $id)->first();
...
public function getWallets($phone)
          {
            $record =LoanWallets::where('AccountID', $phone)->first();
...
public function getLimits($phone)
          {
            $limit = DB::select("SELECT * FROM loan_limits WHERE phone =".$phone." AND orgid =".Auth::user()->orgid." ");
            if($limit){
       ...
       //get branch limits and balances
        public function getBranchInfo($id)
          {
            $b = DB::select("SELECT * FROM branches WHERE id =".$id." AND orgid =".Auth::user()->orgid." ");
            if($b){  
            return $b;
...
public function getRates($amount)
          {
            $rates = DB::select("SELECT * FROM rates WHERE ".$amount." BETWEEN `lower` AND `upper` AND orgid =".Auth::user()->orgid." ");
            if($rates){
            	return $rates[0]->rate;
            }else{
            	return 0;
 ...
 public function quarantors($id)
          {

           $q = DB::select("SELECT * FROM loan_guarantors  WHERE client_id = ".$id." ");
           

            if($q){
                if ($q[0]->loan_id){
            $q2 = DB::select("SELECT * FROM loans  WHERE id = ".$q[0]->loan_id."  AND (status = 'pending' OR status='submitted' OR status='active') ");
            //print_r($q2);exit;
            return $q2;
                    }else{
                            return [];
    ...
    public function check_loans($client_id,$status){
             $q2 = DB::select("SELECT * FROM loans  WHERE client_id = ".$client_id."  AND status = '".$status."' ");
             return $q2;
         }
...
//
      public function getLoans($id)
          {
    //'pending','approved','active','withdrawn','rejected','closed','rescheduled','written_off','overpaid','submitted'
      $q = DB::select("SELECT * FROM loans WHERE client_id = ".$id." AND (status = 'pending' OR status='submitted') ");
            return $q;
    ...
    public function loan_products_linked($id)

          {
            // echo $id;
            $q = DB::select("SELECT * FROM loan_product_linked_credit_checks WHERE loan_product_id = ".$id."  ");
            //print_r($q);exit;
            return $q;
       ...
       if (!empty($request->charges)) {
            ...
            foreach ($request->charges as $key => $value) {
               // echo $value;
                ...
                $charges =(int)$charge+(int)$value;
    
            }

            $charge = $charges;

            //$charge =(int)$this->getRates($request->applied_amount);
            // echo $charge;

        }else{
            $charge = 0;//(int)$this->getRates($request->applied_amount)+;
         // save laona
        }

        $charge =(int)$this->getRates($request->applied_amount)+$charge;
...
// exit();
        ...
        $loan_product = LoanProduct::find($request->loan_product_id);
          ...
          if($loan_product){
                 ...
                 if((int)$request->applied_amount  < (int)$loan_product->minimum_principal || (int)$request->applied_amount  > (int)$loan_product->maximum_principal ){
                \flash(trans_choice("Loan Amount should be greater ".(int)$loan_product->minimum_principal." and less than ".(int)$loan_product->maximum_principal."  ", 1))->error()->important();
        ...
        if((int)$charge > (int)$this->getWallets($client->mobile)){
        \flash(trans_choice("Charge Amount ".$charge."  is greater than your wallet available balance,Current Wallet balance is Kshs. ".$this->getWallets($client->mobile).". Please recharge client account and try again ... ", 1))->error()->important();
            return redirect()->back()->withInput();
        //return redirect('loan/create');
       }
...
//
        ...
        if($request->applied_amount > $this->getLimits($client->mobile)){
        \flash(trans_choice("Applied Amount is greater than your limit, Please enter amount less than or equal to Ksh. ".$this->getLimits($client->mobile)." ", 1))->error()->important();
            return redirect()->back()->withInput();
        //return redirect('loan/create');
       }

        $q =$this->quarantors($request->client_id);

       if($q){
        \flash(trans_choice("You have pending quaranteed loan please complete the client have to complete past loan for this client to borrow new loan", 1))->error()->important();
            return redirect()->back()->withInput();
        //return redirect('loan/create');
       }

    //    echo "<pre>"; print_r($loan_product);echo "<pre>";exit;
        if($loan_product){

            // $credits =$this->loan_products_linked($loan_product->id);
            $credits =$this->loan_products_linked($loan_product->id);
            if($credits){
            ...
            if($value->loan_credit_check_id == 1 || $value->loan_credit_check_id == 2 || $value->loan_credit_check_id == 3 || $value->loan_credit_check_id == 4 || $value->loan_credit_check_id == 5 || $value->loan_credit_check_id == 6 || $value->loan_credit_check_id == 7 || $value->loan_credit_check_id == 8 ){


                if($value->loan_credit_check_id == 1){

                $loans =$this->check_loans($request->client_id,'written_off');
                if($loans){
                     \flash(trans_choice("Client has one or more written off loans ", 1))->error()->important();
                return redirect()->back()->withInput();
                }else{
                ...
                if($value->loan_credit_check_id == 2){
                    //same product outstanding
                $loans =$this->check_loans($request->client_id,'active');
                if($loans){
                     \flash(trans_choice("Client has one or more outstanding loans ", 1))->error()->important();
                return redirect()->back()->withInput();
                }
                }

                if($value->loan_credit_check_id == 3){
                    //client arrears

               $loans_amount =$this->get_arrears($request->client_id);//$this->check_loans($request->client_id,'written_off');
                if($loans_amount > 0){

                     \flash(trans_choice("Client have  an arrears of  ".$loans_amount."  ", 1))->error()->important();
                return redirect()->back()->withInput();
                }
                }

                if($value->loan_credit_check_id == 4){
                    //outstanding laons
                $loans =$this->check_loans($request->client_id,'active');
                if($loans){
                     \flash(trans_choice("Client has one or more outstanding loans ", 1))->error()->important();
                ...
                if($value->loan_credit_check_id == 5){
                    //rejected loans
                      $loans =$this->check_loans($request->client_id,'rejected');
                if($loans){
                     \flash(trans_choice("Client has one or more rejected loans ", 1))->error()->important();
                return redirect()->back()->withInput();
                }
            }
              // if($value->loan_credit_check_id == 6){

              // }
                if($value->loan_credit_check_id == 7){
                    //pending or submitted

               $loans =$this->check_loans($request->client_id,'submitted');
                if($loans){
                     \flash(trans_choice("Client has one or more pending or submitted loans ", 1))->error()->important();
                return redirect()->back()->withInput();
                }
                }
                 if($value->loan_credit_check_id == 8){
                    //pending or submitted

               $loans =$this->check_loans($request->client_id,'approved');
                if($loans){
                     \flash(trans_choice("Client has one or more approved loans ", 1))->error()->important();
                return redirect()->back()->withInput();
                }
                }
    ...
    //'pending','approved','active','withdrawn','rejected','closed','rescheduled','written_off','overpaid','submitted'
            // if($this->getLoans($request->client_id)){
            //   // echo "<pre>";print_r($this->getLoans($request->client_id));echo "<pre>";exit;
            //     if($loan_product->allow_top == 0){
            //      \flash(trans_choice("You have a pending loan please wait for the loan to be processed then try again.Thank you.", 1))->error()->important();
            //      return redirect()->back()->withInput();
        ...
        $loan->orgid = Auth::user()->orgid;


      //get wallet and update wallet also check rates applicable
          if($this->getWallet($request->client_id)){
        
         $wallet = $this->getWallet($request->client_id);
         //updadte both wallets user and company wallets
          $current =(int)$wallet->balance - (int)$charge;
          $wallet_core = (int)$this->getWallet(20220104)->balance +(int)$charge;
     
          LoanWallets::where('CustomerID',$request->client_id)->update(['balance'=>$current]);
          ...
          $permitted_chars = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
            ...
            date_default_timezone_set("Africa/Nairobi");
                ...
                $wallet->CustomerID = $client->id;
                $wallet->balance = -(int)$charge;
                $wallet->AccountID = $client->mobile;
       ...
       $loan->fee = $charge;
        ...
        //         $charge =(int)$loan_charge->amount+(int)$charge;

    
        //     }
        //      $loan->fee = $charge;
        // }else{
        //     $loan->fee = (int)$this->getRates($request->applied_amount);
        //  // save laona
        // }
    ...
    public function reschedule_loan(Request $request, $id)
    {
        ...
        $clients = Client::find($loan->client_id);

        $bal = $this->get_arrears($id);
       //get wallets balance 
        $qq = DB::select("SELECT * FROM loan_product_linked_charges ls INNER JOIN loan_charges l ON  `l`.id = `ls`.loan_charge_id  WHERE `ls`.loan_product_id =".$loan->loan_product_id." ");
            if($qq){
                $amt = 0;
                foreach ($qq as $key => $value) {

                    if($value->loan_charge_type_id == 6){

                        if($value->loan_charge_option_id == 6){
                            $amt += (($value->amount)/100) * $bal;
            ...
            if((int)$amt > (int)$this->getWallets($clients->mobile)){
                \flash(trans_choice("Charge Amount KES ".$amt."  is greater than your wallet available balance,Current Wallet balance is KES. ".$this->getWallets($clients->mobile).". Please recharge client account and try again ... ", 1))->error()->important();
                
                return redirect()->back()->withInput();
            }
    ///get wallet and update wallet also check rates applicable
        // dd([$this->getWallets($clients->mobile), $amt]);
        if($this->getWallet($loan->client_id)){
        
         $wallet = $this->getWallets($clients->mobile);
         //updadte both wallets user and company wallets
          $current =(int)$wallet - (int)$amt;
          $wallet_core = (int)$this->getWallet(20220104)->balance +(int)$amt;
     
          LoanWallets::where('CustomerID',$loan->client_id)->update(['balance'=>$current]);
          LoanWallets::where('CustomerID',20220104)->update(['balance'=>$wallet_core]);
          $permitted_chars = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
          ...
          date_default_timezone_set("Africa/Nairobi");
            ...
            DB::insert('INSERT INTO wallet_history
                ...
                \flash(trans_choice("No wallet found for this client", 1))->error()->important();
                
                return redirect()->back()->withInput();
            // date_default_timezone_set("Africa/Nairobi");
            //     //$current =(int)$wallet->balance -(int)$this->getRates($request->applied_amount);
            //     $wallet = new LoanWallets();
            //     $wallet->CustomerID = $clients->id;
            //     $wallet->balance = -(int)$amt;
            //     $wallet->AccountID = $clients->mobile;
            //     $wallet->created_on = date("Y-m-d h:i:s");
            //     $wallet->updated_at = date("Y-m-d h:i:s");
            //     $wallet->save();
         }
        ...
        $balance = round($bal, $loan->decimals);
        ...
        for ($i = 1; $i <= $period; $i++) {
            ...
            if ($loan->interest_methodology == 'flat') {
                ...
                $principal = round($bal / $period, $loan->decimals);
            ...
            if ($loan->interest_methodology == 'declining_balance') {
                if ($loan->amortization_method == 'equal_installments') {
                    ...
                    $amortized_payment = round(determine_amortized_payment($interest_rate, $bal, $period), $loan->decimals);
    ...
    public function approve_loan(Request $request, $id)
    {
        ...
        $loan_product = LoanProduct::find($loan->loan_product_id);
       
        if((int)$loan_product->guarantor == 1){

        $q = DB::select("SELECT * FROM loan_guarantors  WHERE loan_id = ".$id." ");
        if($q){
            
            foreach ($q as $key => $value) {

                 if($value->status =="pending" ){
                     \flash(trans_choice("One or more quarantors for this loan not activated ,please activated them ans try try again. ", 1))->error()->important();
                     return redirect()->back();
              }
            }
         
          }else{
             \flash(trans_choice("You have not provided required loan guarantors for this loan..", 1))->error()->important();
             return redirect()->back();
          }
        }

        $client_user = Client::find($loan->client_id);

   
        // echo '<pre>'; print_r($client_user);
        // exit;
        ...
        $loan_history->save();
          ...
          ////updated loan issued/approved transactions

         $eq = $this->getBranchInfo($client_user->branch_id);
        if(!empty($eq)){
        $cam=(int)$eq[0]->utility_acc - (int)$request->approved_amount;
        $loant = new LoanTrans();
        $loant->client_id = $client_user->id;
        $loant->branch_id = $client_user->branch_id;
        $loant->loan_officer_id = $client_user->loan_officer_id;
        $loant->created_by_id = Auth::id();
        $loant->applied_amount =(int) $request->approved_amount;
        //$loant->update = date("Y-m-d H:i:s");
        $loant->created_at = date("Y-m-d H:i:s");
        $loant->c_amount  =$cam;
        $loant->p_amount  =(int)$eq[0]->utility_acc;
        $loant->orgid  = Auth::user()->orgid;
        $loant->status  =0;
        $loant->loan_id  =$loan->id;
        $loant->save();
        //update utility account

        DB::table('branches')->where('id', $client_user->branch_id)->update(['utility_acc' => $cam,'weq' => $cam]);

    }

         

          $msg ="Dear ".$client_user->first_name." ".$client_user->last_name." your loan of KES ".(int)$loan->approved_amount." has been approved,we will notify you when we disburse it.Thank you for being our valued customer.";
        //$this->sendSms($client_user->mobile,$msg);
        activity()->on($loan)
            ->withProperties(['id' => $loan->id])
            ->log('Approve Loan');
        //fire loan status changed event
    ...
    public function undo_approval(Request $request, $id)
    {
        ...
        $client_user = Client::find($loan->client_id);
         ...
         $eq = $this->getBranchInfo($client_user->branch_id);
        if(!empty($eq)){
        $cam=(int)$eq[0]->utility_acc + (int)$loan->applied_amount;
        ...
        //update and recall the amount undone
        DB::table('loan_trans')->where(['loan_id'=>$loan->id, 'branch_id'=>$client_user->branch_id])->update(['c_amount' => $cam,'p_amount' => $cam,'status'=>2]);
         }
          $msg ="Dear ".$client_user->first_name." ".$client_user->last_name." your loan has been cancelled,please contact us for more details.Thank you for being our valued customer.";
        $this->sendSms($client_user->mobile,$msg);
        activity()->on($loan)
            ->withProperties(['id' => $loan->id])
    ...
    public function reject_loan(Request $request, $id)
    {
        ...
        $client_user = Client::find($loan->client_id);
         ...
         $q = DB::select("SELECT * FROM loan_guarantors  WHERE loan_id = ".$id." ");
           ...
           if($q){
            DB::table('loan_guarantors')->where('loan_id', $id)->update(['status' =>'inactive']);
        }
        ...
        //  }
          ...
          // $msg ="Dear ".$client_user->first_name." ".$client_user->last_name." your loan was rejected,Because of the following ".$loan->rejected_notes.".Thank you for being our valued customer.";
        activity()->on($loan)
            ->withProperties(['id' => $loan->id])
            ->log('Reject Loan');
        //fire loan status changed event
    ...
    public function undo_rejection(Request $request, $id)
    {
        ...
        $q = DB::select("SELECT * FROM loan_guarantors  WHERE loan_id = ".$id." ");
           ...
           if($q){
            DB::table('loan_guarantors')->where('loan_id', $id)->update(['status' =>'active']);
            }
        ...
        //  }
          ...
          $msg ="Dear ".$client_user->first_name." ".$client_user->last_name." your loan rejection was undone,your loan application will now proceed to the next stage.Thank you for being our valued customer.";
    ...
    public function closed_loan(Request $request, $id)
    {
        ...
        //echo "<pre>";echo $balance; exit;
        if($balance > 0){
          \flash(trans_choice("Loan Balance should be 0 for it to be closed ", 1))->error()->important();
          return redirect('loan/' . $loan->id . '/show');
         }
        ...
        if ($loan->loan_product->accounting_rule == "cash" || $loan->loan_product->accounting_rule == "accrual_periodic" || $loan->loan_product->accounting_rule == "accrual_upfront") {
            ...
            $journal_entry->currency_id = $loan->currency_id;
            $journal_entry->chart_of_account_id = $loan->loan_product->loan_portfolio_chart_of_account_id;
            $journal_entry->transaction_type = 'closed';
            $journal_entry->date = $loan->written_off_on_date;
            $date = explode('-', $loan->written_off_on_date);
            $journal_entry->month = $date[1];
            $journal_entry->year = $date[0];
    ...
    public function b2c($phone, $amt){
          ...
          $curl_post_data = array(
            ...
            'InitiatorName' => 'josam.api',
            'SecurityCredential' => '',
            'CommandID' => 'BusinessPayment',
            'PartyA' => '3031851',
            'Amount' =>$amount,
            'PartyB' => $phone,
            'Remarks' => 'Customer Deposits',
            'QueueTimeOutURL' => 'https://sacco.josamgroup.com/api/callback/urlcheck',
            'ResultURL' => 'https://sacco.josamgroup.com/api/callback/saveresponse',
       ...
       //print_r($curl_post_data);exit;
        ...
        $consumerkey    ="eLrHgrBrxQoo4N52pOjNRGMQJPhOhVbe";
        $consumersecret ="UiXsQMA8w2OGJPza";
    ...
    public function disburse_loan(Request $request, $id)
    {
        ...
        $receipt =substr(str_shuffle($permitted_chars), 0, 10);
        //payment details
        ...
        $payment_detail->receipt =$receipt;// $request->receipt;
       ...
       // exit;
         ...
         //update and recall the amount undone
        DB::table('loan_trans')->where(['loan_id'=>$loan->id, 'branch_id'=>$client_user->branch_id])->update(['status'=>1]);
        ...
        $total =((float)$loan->principal +(float)$total_interest);
         //echo $total;exit;
        $msg="Dear ".$client_user->first_name." ".$client_user->last_name.",You have received KES ".number_format((float)$loan->principal, 2, '.', '')." on your Acc ".$client_user->mobile.". Ref No. is ".$receipt." Your Deposit Account balance is KES 0.00 .Your outstanding loan balance is KES ".number_format((float)$total, 2, '.', '').". Your loan is due on ".$request->first_payment_date."";
        if((int)$request->payment_type_id === 1){
             //$this->b2c($client_user->mobile,100);
              $this->sendSms($client_user->mobile,$msg);
        }else{
              $this->sendSms($client_user->mobile,$msg);
        }
        ...
        $loan_transaction->save();
                 ...
                 if((int)$balance > 0 ){
                        ...
                        // }
                          ...
                          $this->update_wallets($client->mobile,$balance,'N/A',$client->mobile);
                          DB::table('loans')->where('id', $loan->id)->update(['status' =>'closed']);
                          $q = DB::select("SELECT * FROM loan_guarantors  WHERE loan_id = ".$loan->id." ");
                          if($q){
                            DB::table('loan_guarantors')->where('loan_id', $loan->id)->update(['status' =>'closed']);
                       ...
                       // $this->update_wallets($client->mobile,$bal1,'N/A',$client->mobile);
                            DB::table('loans')->where('id', $loan->id)->update(['status' =>'closed']);
                          $q = DB::select("SELECT * FROM loan_guarantors  WHERE loan_id = ".$loan->id." ");
                          if($q){
                            DB::table('loan_guarantors')->where('loan_id', $loan->id)->update(['status' =>'closed']);
                        ...
                        $this->update_wallets($client->mobile,$amount,'N/A',$client->mobile);

                    }

        $msg ="Dear ".$client->first_name.' '.$client->last_name.", we have received your repayment of KES.  ".number_format((float)$amount, 2, '.', '')." on ".$request->date.",Your outstanding loan balance is KES ".((int)$balance > 0 ? number_format((float)$balance, 2, '.', ''): 0.00)." your loan is due on ".$loan->expected_first_payment_date.".";
     ...
     public function update_wallets($phone,$amount,$trans_code,$msisdn)
    {
      ...
      if($this->getWallets($phone)){
        
         $wallet = $this->getWallets($phone);
         //updadte both wallets user and company wallets
          $current = (int)$wallet + (int)$amount;
    ...
    public function process_loan_calculator(Request $request)
    {
        ...
        $loan_details['total_fees'] = $disbursement_fees + $installment_fees;
         $loan_details['total_due'] = $installment_fees + $total_interest + $total_principal;
