...
class LoanProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = LoanProduct::leftJoin('currencies', 'currencies.id', 'loan_products.currency_id')
            ...
            if(count($r) === 0 ){
                $query->where('loan_products.orgid', $bid);
                }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'default_interest_rate' => ['required', 'numeric', 'lt:maximum_interest_rate'],
        ...
        $loan_product->guarantor = $request->guarantor;
        $loan_product->orgid =Auth::user()->orgid;
        $loan_product->save();
        //save charges
        if (!empty($request->charges)) {
            foreach (explode(',',$request->charges) as $key) {
                if (!empty($key)) {
                    ...
                    $loan_product_charge->orgid =Auth::user()->orgid;
        ...
        if (!empty($request->credit_checks)) {
            ...
            foreach (explode(',',$request->credit_checks) as $key) {
                if (!empty($key)) {
                    ...
                    $loan_product_credit_check->orgid =Auth::user()->orgid;
    ...
    public function edit($id)
    {
        ...
        $credit_checks = LoanCreditCheck::where('active', 1)->get();
        $loan_charges = LoanCharge::where('active', 1)->get();
        $checks = LoanProductLinkedCreditCheck::where('loan_product_id', $id)->get();
        $selected_credit_checks = [];
         foreach ($checks as $key) {
            $selected_credit_checks[] = $key->loan_credit_check_id;
        }
        $selected_charges = [];
        ...
        return theme_view('loan::loan_product.edit', compact('funds', 'currencies', 'expenses', 'income', 'liabilities', 'assets', 'credit_checks', 'loan_charges', 'loan_transaction_processing_strategies', 'loan_product', 'selected_charges','selected_credit_checks'));
    }
